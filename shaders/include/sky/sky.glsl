...
vec3 stable_star_field(vec2 coord, float star_threshold) {
	...
	return unstable_star_field(i + vec2(0.0, 0.0), star_threshold) * (15.0 - f.x) * (15.0 - f.y)
	     + unstable_star_field(i + vec2(1.0, 0.0), star_threshold) * f.x * (20.0 - f.y)
	     + unstable_star_field(i + vec2(0.0, 1.0), star_threshold) * f.y * (15.0 - f.x)
	     + unstable_star_field(i + vec2(1.0, 1.0), star_threshold) * f.x * f.y;
}

vec3 draw_stars(vec3 ray_dir) {
    // 上部のみに星を描画するための閾値を設定します。
    // この値は0から1の範囲で、1に近いほど星は上部に集中します。
    const float upper_threshold = 0.1;

    // 雨が降っているかどうかを示すブール値を定義します。
    // この値は、実際の状況に応じて適切に設定する必要があります。
    bool isRaining = false; // ここを適切に設定してください。

    // レイの方向が閾値以上の場合にのみ星を描画します。
    // さらに、雨が降っていない場合にのみ星を描画します。
    if (ray_dir.y > upper_threshold && !isRaining) {
        // 以下は元のコードと同じです。
        ...
        float star_threshold = 1.0 - 0.003 * STARS_COVERAGE * smoothstep(-0.2, 0.05, -sun_dir.y);
#else
        float star_threshold = 1.0 - 0.003 * STARS_COVERAGE;
#endif
        ...
        // 閾値以下の場合、または雨が降っている場合は、星を描画せずに黒を返します。
...
const float moon_luminance = 40.0; // luminance of moon disk
...
vec3 draw_sun(vec3 ray_dir) {
	...
	const vec3 alpha = vec3(0.450, 0.530, 0.620);
	...
	// Smooth the edge of the sun
	float smooth_edge = smoothstep(0.0, 0.1, center_to_edge);

	return sun_luminance * sun_color * smooth_edge * limb_darkening;
}
...
vec3 draw_sky(vec3 ray_dir, vec3 atmosphere) {
	...
	// Rain
	vec3 rain_sky = get_weather_color() * (1.00 - exp2(-0.8 / clamp01(ray_dir.y)));
