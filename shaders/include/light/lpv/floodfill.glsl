...
const float saturation_factor = (1.0 * EMITTED_LIGHT_SATRATION); // Default saturation factor
...
vec3 hsv2rgb(vec3 c) {
    ...
    return c.z * clamp(mix(K.xxx, p - K.xxx, c.y), 0.0, 1.0); // 修正
}

vec3 adjust_saturation(vec3 color, float saturation_factor) {
    vec3 hsv = rgb2hsv(color);
    hsv.y *= saturation_factor; // adjust saturation
    return hsv2rgb(hsv);
}
...
void update_lpv(writeonly image3D light_img, sampler3D light_sampler) {
    ...
    // Adjust the saturation of the emitted light using the uniform saturation_factor
    emitted_light = adjust_saturation(emitted_light, saturation_factor);
